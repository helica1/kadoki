/*
 * yomitan-deinflect.js
 *
 * Self-contained port of Yomitan's Japanese deinflector for the Anki Deck
 * Reader Capacitor shell. Bundles the rule-table from
 * ext/js/language/ja/japanese-transforms.js plus a trimmed-down
 * LanguageTransformer engine. Source pinned at upstream commit
 * 3af775bda1df7580f39f9e2348ddddf09a232968 (yomidevs/yomitan, master).
 *
 * Yomitan is GPL-3.0-or-later, Copyright (C) 2024-2026 Yomitan Authors.
 * See https://github.com/yomidevs/yomitan/blob/master/COPYING.
 *
 * Public API attached to window:
 *   window.yomitanGetDeinflections(surface, maxDepth = 3)
 *     -> [{ word, reason, depth }]
 *     The original surface is always returned at depth 0.
 *   window.getDeinflections        (alias of the above)
 *   window.yomitanDeinflector      (engine for diagnostics)
 *
 * Plain ES2017, no imports, no fetches, no top-level await.
 */
...
(function () {
    ...
    // ---------------------------------------------------------------
    // Rule helpers (ported from ext/js/language/language-transforms.js)
    // ---------------------------------------------------------------

    function suffixInflection(inflectedSuffix, deinflectedSuffix, conditionsIn, conditionsOut) {
        var suffixRegExp = new RegExp(inflectedSuffix + '$');
        var inflLen = inflectedSuffix.length;
        return {
            type: 'suffix',
            isInflected: suffixRegExp,
            deinflected: deinflectedSuffix,
            deinflect: function (text) { return text.slice(0, -inflLen) + deinflectedSuffix; },
    ...
    function wholeWordInflection(inflectedWord, deinflectedWord, conditionsIn, conditionsOut) {
        var regex = new RegExp('^' + inflectedWord + '$');
        return {
            type: 'wholeWord',
            isInflected: regex,
            deinflect: function () { return deinflectedWord; },
            conditionsIn: conditionsIn,
            conditionsOut: conditionsOut,
        };
    }
    ...
    // ---------------------------------------------------------------
    // Begin pasted body of ext/js/language/ja/japanese-transforms.js
    // (GPL-3, Yomitan Authors). The `import` line has been stripped
    // and `export const japaneseTransforms` has been rewritten as a
    // local const declaration. Everything else is byte-for-byte
    // identical to the upstream source.
    // ---------------------------------------------------------------
...
const japaneseTransforms = {
    ...
    // ---------------------------------------------------------------
    // End pasted body. Resume LanguageTransformer engine.
    // ---------------------------------------------------------------

    // Compact LanguageTransformer ported from
    // ext/js/language/language-transformer.js. We only need the
    // forward `transform()` direction (deinflection), but we retain
    // condition-flag logic so multi-step chains (e.g. -ています ->
    // -ている -> -る) honor the same parts-of-speech gating that
    // Yomitan uses upstream.

    function LanguageTransformer() {
    ...
    LanguageTransformer.prototype.addDescriptor = function (descriptor) {
        var conditions = descriptor.conditions;
        var transforms = descriptor.transforms;
        var conditionEntries = Object.keys(conditions).map(function (k) { return [k, conditions[k]]; });
        var fm = this._getConditionFlagsMap(conditionEntries, this._nextFlagIndex);
        var conditionFlagsMap = fm.conditionFlagsMap;
        var nextFlagIndex = fm.nextFlagIndex;
        var transforms2 = [];
        var tIds = Object.keys(transforms);
        for (var t = 0; t < tIds.length; ++t) {
            var transformId = tIds[t];
            var transform = transforms[transformId];
            var rules = transform.rules;
            var rules2 = [];
            for (var j = 0, jj = rules.length; j < jj; ++j) {
                var rule = rules[j];
                var conditionFlagsIn = this._getConditionFlagsStrict(conditionFlagsMap, rule.conditionsIn);
                if (conditionFlagsIn === null) { throw new Error('Invalid conditionsIn for transform ' + transformId + '.rules[' + j + ']'); }
                var conditionFlagsOut = this._getConditionFlagsStrict(conditionFlagsMap, rule.conditionsOut);
            ...
            var heuristicSrc = rules.map(function (r) { return r.isInflected.source; }).join('|');
            transforms2.push({
                id: transformId,
                name: transform.name,
                description: transform.description,
                rules: rules2,
                heuristic: new RegExp(heuristicSrc),
            });
        }
        this._nextFlagIndex = nextFlagIndex;
        for (var ti = 0; ti < transforms2.length; ++ti) {
            this._transforms.push(transforms2[ti]);
        }
        for (var ci = 0; ci < conditionEntries.length; ++ci) {
            var entry = conditionEntries[ci];
            var type = entry[0];
            var condition = entry[1];
            var flags = conditionFlagsMap.get(type);
    ...
    LanguageTransformer.prototype._getConditionFlagsMap = function (conditions, nextFlagIndex) {
        var conditionFlagsMap = new Map();
        var targets = conditions.slice();
        while (targets.length > 0) {
            var nextTargets = [];
            for (var i = 0; i < targets.length; ++i) {
                var target = targets[i];
                var type = target[0];
                var condition = target[1];
                ...
                if (typeof subConditions === 'undefined') {
                    ...
                    var multiFlags = this._getConditionFlagsStrict(conditionFlagsMap, subConditions);
                    if (multiFlags === null) {
            ...
            if (nextTargets.length === targets.length) {
                throw new Error('Cycle in subCondition declaration');
            }
            targets = nextTargets;
        }
    ...
    LanguageTransformer.prototype._getConditionFlagsStrict = function (conditionFlagsMap, conditionTypes) {
        var flags = 0;
        for (var i = 0; i < conditionTypes.length; ++i) {
            var flags2 = conditionFlagsMap.get(conditionTypes[i]);
    ...
    LanguageTransformer.conditionsMatch = function (currentConditions, nextConditions) {
        return currentConditions === 0 || (currentConditions & nextConditions) !== 0;
    };

    // Returns: [{ text, conditions, trace }]. trace[0] is the most
    // recent transform applied. trace.length === depth.
    LanguageTransformer.prototype.transform = function (sourceText, maxDepth) {
        if (typeof maxDepth !== 'number' || maxDepth < 0) { maxDepth = Infinity; }
        var results = [{ text: sourceText, conditions: 0, trace: [] }];
        for (var i = 0; i < results.length; ++i) {
            ...
            for (var t = 0; t < this._transforms.length; ++t) {
                var transform = this._transforms[t];
                ...
                for (var j = 0, jj = rules.length; j < jj; ++j) {
                    ...
                    // Cycle guard: same transform+rule+text seen before in this chain.
                    var isCycle = false;
                    for (var k = 0; k < trace.length; ++k) {
                        var frame = trace[k];
                        if (frame.transform === transform.id && frame.ruleIndex === j && frame.text === text) {
                            isCycle = true;
                            break;
                        }
                    }
                    if (isCycle) { continue; }
                    var newText = rule.deinflect(text);
                    if (!newText || newText.length < 1) { continue; }
                    var newTrace = [{ transform: transform.id, ruleIndex: j, text: text }];
                    for (var ti2 = 0; ti2 < trace.length; ++ti2) { newTrace.push(trace[ti2]); }
                    results.push({ text: newText, conditions: rule.conditionsOut, trace: newTrace });
                }
            }
        }
        return results;
    ...
    // ---------------------------------------------------------------
    // Build the singleton transformer and expose the public API.
    // ---------------------------------------------------------------

    var transformer = new LanguageTransformer();
    transformer.addDescriptor(japaneseTransforms);

    // Count total rules for diagnostics.
    var __ruleCount = 0;
    (function () {
        var transformIds = Object.keys(japaneseTransforms.transforms);
        for (var i = 0; i < transformIds.length; ++i) {
            __ruleCount += japaneseTransforms.transforms[transformIds[i]].rules.length;
        }
    })();

    function yomitanGetDeinflections(surface, maxDepth) {
        if (typeof maxDepth !== 'number') { maxDepth = 3; }
        if (typeof surface !== 'string' || surface.length === 0) {
            return [{ word: surface, depth: 0, reason: null }];
        }
        var transformed = transformer.transform(surface, maxDepth);
        // De-duplicate by word, keep the shallowest depth for each.
        var seen = new Map();
        for (var i = 0; i < transformed.length; ++i) {
            var entry = transformed[i];
            var depth = entry.trace.length;
            var existing = seen.get(entry.text);
            if (existing && existing.depth <= depth) { continue; }
            // `reason` mirrors the original deinflector's intent: a short
            // human-readable hint about what was unwound to get here.
            // We use the most-recent transform id (e.g. '-て', '-た').
            var reason = depth > 0 ? entry.trace[0].transform : null;
            seen.set(entry.text, { word: entry.text, depth: depth, reason: reason });
        }
        // Guarantee the original surface is in the output at depth 0
        // (LanguageTransformer.transform() already puts it there, but
        // belt-and-suspenders for the contract enhanced-dictionary.js relies on).
        if (!seen.has(surface)) {
            seen.set(surface, { word: surface, depth: 0, reason: null });
        }
        return Array.from(seen.values());
    }

    window.yomitanGetDeinflections = yomitanGetDeinflections;
    window.yomitanDeinflector = {
        transformer: transformer,
        descriptor: japaneseTransforms,
        ruleCount: __ruleCount,
        version: 'yomitan@3af775b',
    };

    // Friendly load-time log so we can confirm in the WebView console.
    try {
        console.log('[yomitan-deinflect] loaded; ruleCount=' + __ruleCount +
            ' transforms=' + Object.keys(japaneseTransforms.transforms).length);
    } catch (_) { /* console may be unavailable in some contexts */ }
})();
